...
def main():
    ...
    # Create figure
    # 3 rows (K=1, 4, 16), 3 columns (Exact, PINN, DD)
    # This allows side-by-side comparison of solution and predictions for each K
    ...
    for i, K in enumerate(Ks):
        ...
        # Use same vmin/vmax for each row (each K) to make comparison valid
        vmin = np.min(sol)
        vmax = np.max(sol)
        ...
        for j, data in enumerate(data_list):
            ...
            if i == 0:
                ax.set_title(methods[j], fontweight='bold', fontsize=18)
            
            # Set row labels (K values)
            if j == 0:
                ax.set_ylabel(f'K = {K}', fontweight='bold', fontsize=18)
            
            # Remove ticks to save space
            ax.set_xticks([])
            ax.set_yticks([])
            
            # Add colorbar for each plot? Or one per row?
            # One per row might be cleaner if they share the scale
